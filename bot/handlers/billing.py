...
from bot.utils import api_call, get_user_lang, render_screen
...
async def _execute_invoice_creation(callback_query: CallbackQuery, tariff_slug: str, currency: str = "RUB"):
    ...
    # Гасим "часики" на кнопке, пока идёт запрос
    try:
    ...
    if result.get("status") == "payment_required":
        ...
        await render_screen(callback_query, text=msg, reply_markup=pay_keyboard)
        return
...
async def show_clean_tariffs(event):
    ...
    if not raw_tariffs or raw_tariffs.get("state") == "error":
        ...
        if is_callback:
            await render_screen(event, text=msg_text)
            await event.answer()
        else:
    ...
    tariffs_list = [
        {"slug": k, "name_ru": v.get("name_ru", k), "name_en": v.get("name_en", k)}
        for k, v in raw_tariffs.items()
    ]
    display_msg = loc["tariff_title"]
    reply_kb = KEYBOARDS["tariffs"](tariffs=tariffs_list, lang=lang)

    if is_callback:
        await render_screen(event, text=display_msg, reply_markup=reply_kb)
        await event.answer()
    else:
...
async def process_tariff_click(callback_query: CallbackQuery):
    """ЭКРАН 2: либo запрашиваем валюту, либo сразу создаём инвойс."""
    lang = await get_user_lang(callback_query)
...
async def process_currency_selected(callback_query: CallbackQuery):
    """Запасной экран выбора валюты (активен, только если USE_CURRENCY_SCREEN = True)."""
