...
Поддерживает сводку, детализацию (-v) и сверку с Hiddify (--sync).
"""
...
# Загружаем переменные из .env, чтобы HOST_API_KEY был доступен
load_dotenv()
...
@click.option('--sync', is_flag=True, help="Выполнить сверку с Hiddify (может занять некоторое время)")
def check(query, verbose, sync):
    ...
    С флагом --sync — выполняет реальную сверку с панелью Hiddify.

    Примеры:
        uadmin check
        uadmin check -v
        uadmin check --sync
        uadmin check 880765948
    """
    async def _run():
        if query:
            await _check_entity(query)
        else:
            await _check_summary(verbose, sync)
...
async def _check_summary(verbose: bool, sync: bool = False):
    """Полная сводка аномалий (через API)."""
    headers = {"X-API-Key": API_KEY}
    params = {}
    if verbose:
        params["verbose"] = "true"
    if sync:
        params["hiddify_sync"] = "true"
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            mismatches = summary.get("status_mismatches_count", 0)
            hiddify_anomalies = summary.get("hiddify_anomalies_count", 0)

            sum_table.add_row("🗑️ Устаревшие инвойсы", str(dirty_invoices))
            sum_table.add_row("🚨 Зависшие активации", str(failed_provisioning))
            sum_table.add_row("🔄 Расхождения статусов с Hiddify", str(mismatches))
            sum_table.add_row("🌍 Критические аномалии профилей", str(hiddify_anomalies))
            console.print(sum_table)

            # Детализация при verbose: списки инвойсов и зависших подписок
            if verbose:
                dirty_list = data.get("dirty_invoices", [])
                if dirty_list:
                    ...
                    for inv in dirty_list:
                        inv_table.add_row(
                            inv.get("id", "-"),
                            inv.get("email", "-"),
                            inv.get("tariff_slug", "-"),
                            f"{inv.get('amount', 0)} RUB",
                            inv.get("created_at", "-")
                        )
                    console.print(inv_table)

                failed_list = data.get("failed_provisioning_list", [])
                if failed_list:
                    ...
                    for item in failed_list:
                        ...
                        sub_table.add_row(
                            ...
                            str(item.get("last_error", "Ожидает повтора"))
                        )
                    console.print(sub_table)

            # Результаты сверки с Hiddify (если выполнялась)
            mismatches_list = data.get("status_mismatches", [])
            if mismatches_list:
            ...
            if anomalies_list:
                ...
                an_table.add_column("Подписка", style="magenta")      # новый столбец
            ...
            if total == 0:
                ...
                console.print("[dim]uadmin check --sync        — свериться с Hiddify[/dim]")
...
async def _check_entity(query: str):
    """Детализация по конкретной сущности (через API)."""
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if account:
                ...
                acc_table.add_row("ID", str(account.get("id", "—")))
            ...
            if subscription:
                ...
                console.print("[yellow]⚠️ Подписка отсутствует[/yellow]")

            # Аномалия (пока всегда None, но оставим заглушку)
            anomaly = data.get("anomaly")
            if anomaly:
                console.print(f"\n[bold red]⚠️ Аномалия:[/bold red] {anomaly}")
            else:
                console.print("\n[green]✅ Аномалий не выявлено.[/green]")
