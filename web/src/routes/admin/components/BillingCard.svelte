...
<div class="w-64 bg-slate-900 p-5 overflow-y-auto shrink-0 space-y-4">
    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400 border-b border-slate-800 pb-2 m-0">📊 подписка </h3>
    {#if loading}
          ...
          {:else}
            <div class="space-y-4 text-sm">
              <div>
                ...
                <code class="bg-slate-950 text-blue-400 px-2 py-1 rounded block text-xs truncate_ select-all">{info.hiddify_uuid || '—'}</code>
              </div>
